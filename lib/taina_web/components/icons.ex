defmodule TainaWeb.Icons do
  @moduledoc """
  Ícones do design system (página "01 - Componentes" do Penpot) como SVG
  inline, sem icon font, sem requisição extra. Os paths são o conjunto
  outline do Lucide (ISC), 24x24, stroke 2, pontas arredondadas.

  Uso via `TainaWeb.CoreComponents.icon/1`; este módulo só guarda os paths.
  """
  ...
  @paths %{
    ...
    "folder-plus" =>
      ~S(<path d="M12 10v6"/><path d="M9 13h6"/><path d="M20 20a2 2 0 0 0 2-2V8a2 2 0 0 0-2-2h-7.9a2 2 0 0 1-1.69-.9L9.6 3.9A2 2 0 0 0 7.93 3H4a2 2 0 0 0-2 2v13a2 2 0 0 0 2 2Z"/>),
    "image" =>
    ...
    "upload" =>
      ~S(<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" x2="12" y1="3" y2="15"/>),
    "download" =>
    ...
    "list" =>
      ~S(<line x1="8" x2="21" y1="6" y2="6"/><line x1="8" x2="21" y1="12" y2="12"/><line x1="8" x2="21" y1="18" y2="18"/><line x1="3" x2="3.01" y1="6" y2="6"/><line x1="3" x2="3.01" y1="12" y2="12"/><line x1="3" x2="3.01" y1="18" y2="18"/>),
    "check" => ~S(<path d="M20 6 9 17l-5-5"/>),
    "close" => ~S(<path d="M18 6 6 18"/><path d="m6 6 12 12"/>),
    "clock" => ~S(<circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>),
    "qr" =>
      ~S(<rect width="5" height="5" x="3" y="3" rx="1"/><rect width="5" height="5" x="16" y="3" rx="1"/><rect width="5" height="5" x="3" y="16" rx="1"/><path d="M21 16h-3a2 2 0 0 0-2 2v3"/><path d="M21 21v.01"/><path d="M12 7v3a2 2 0 0 1-2 2H7"/><path d="M3 12h.01"/><path d="M12 3h.01"/><path d="M12 16v.01"/><path d="M16 12h1"/><path d="M21 12v.01"/><path d="M12 21v-1"/>),
    "link" =>
    ...
    "restore" => ~S(<path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/>),
    "spark" =>
      ~S(<path d="M9.937 15.5A2 2 0 0 0 8.5 14.063l-6.135-1.582a.5.5 0 0 1 0-.962L8.5 9.936A2 2 0 0 0 9.937 8.5l1.582-6.135a.5.5 0 0 1 .963 0L14.063 8.5A2 2 0 0 0 15.5 9.937l6.135 1.581a.5.5 0 0 1 0 .964L15.5 14.063a2 2 0 0 0-1.437 1.437l-1.582 6.135a.5.5 0 0 1-.963 0z" fill="currentColor" stroke="none"/>),
    ...
    "move" => ~S(<polyline points="15 14 20 9 15 4"/><path d="M4 20v-7a4 4 0 0 1 4-4h12"/>),
    "disk" =>
  ...
  @doc "Nomes de ícone disponíveis (para validação em `attr :name, values: ...`)."
  def names, do: @names

  @doc "Markup interno (paths) do ícone. Levanta se o nome não existir."
  def path!(name), do: Map.fetch!(@paths, name)
end
