defmodule TainaWeb.SessionControllerTest do
  ...
  setup do
    tekoa = tekoa_fixture()
    ava = confirmed_ava_fixture(tekoa, %{password: @password})
    %{tekoa: tekoa, ava: ava}
  end

  test "login com credenciais válidas cria sessão e vai para a home", %{conn: conn, ava: ava} do
    conn = post(conn, ~p"/login", %{"email" => ava.email, "password" => @password})
  ...
  test "senha errada volta para o login com aviso", %{conn: conn, ava: ava} do
    conn = post(conn, ~p"/login", %{"email" => ava.email, "password" => "senha-errada-123"})
